{{-- resources/views/crm/staff/laporan/salary.blade.php --}}
@extends('layouts.app')
...
@push('page-actions')
    <div class="flex gap-2">
        <a href="{{ route('administrasi.laporan.export.pdf', 'salary') }}?{{ $exportQuery ?? '' }}" class="px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
            <i class="fa-solid fa-file-pdf mr-1"></i> PDF
        </a>
        <a href="{{ route('administrasi.laporan.export.excel', 'salary') }}?{{ $exportQuery ?? '' }}" class="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
            <i class="fa-solid fa-file-excel mr-1"></i> Excel
        </a>
        <a href="{{ route('administrasi.laporan.export.word', 'salary') }}?{{ $exportQuery ?? '' }}" class="px-3 py-2 bg-blue-700 text-white rounded-lg hover:bg-blue-800">
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
    <form method="GET" action="{{ route('administrasi.laporan.salary') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <div>
        ...
        <div class="lg:col-span-6 flex items-center gap-2 pt-2">
            ...
            <a href="{{ route('administrasi.laporan.salary') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div class="text-center">
            <p class="text-3xl font-bold text-blue-600">{{ number_format($summary['total_employees']) }}</p>
            <p class="text-sm text-gray-500">Total Karyawan</p>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div class="text-center">
            <p class="text-3xl font-bold text-green-600">Rp {{ number_format($summary['total_salary'], 0, ',', '.') }}</p>
            <p class="text-sm text-gray-500">Total Gaji</p>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div class="text-center">
            <p class="text-3xl font-bold text-amber-600">Rp {{ number_format($summary['total_deductions'], 0, ',', '.') }}</p>
            <p class="text-sm text-gray-500">Total Potongan</p>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div class="text-center">
            <p class="text-3xl font-bold text-emerald-600">Rp {{ number_format($summary['total_salary'] - $summary['total_deductions'], 0, ',', '.') }}</p>
            <p class="text-sm text-gray-500">Total Bersih</p>
        </div>
    </div>
</div>
...
<div class="bg-emerald-50 rounded-xl border border-emerald-200 p-4 mb-6">
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-lg font-semibold text-emerald-900">Laporan Gaji</h3>
            <p class="text-sm text-emerald-700">{{ $monthName ?? '' }} {{ $filters['year'] ?? date('Y') }}</p>
        </div>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-10 w-10">
                                <img class="h-10 w-10 rounded-full" src="https://ui-avatars.com/api/?name={{ urlencode($sal['employee_name']) }}&background=667eea&color=fff" alt="">
                            </div>
                            <div class="ml-4">
                                <div class="text-sm font-medium text-gray-900">{{ $sal['employee_name'] }}</div>
                                <div class="text-sm text-gray-500">{{ $sal['employee_id'] }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $sal['department'] ?? '-' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-900">Rp {{ number_format($sal['basic_salary'] ?? 0, 0, ',', '.') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-green-600">Rp {{ number_format($sal['allowances'] ?? 0, 0, ',', '.') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-red-600">Rp {{ number_format($sal['deductions'] ?? 0, 0, ',', '.') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-bold text-gray-900">Rp {{ number_format($sal['total_salary'] ?? 0, 0, ',', '.') }}</td>
                ...
                <tr>
                    <td colspan="8" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center">
                            <i class="fa-solid fa-money-bill-wave text-gray-300 text-4xl mb-3"></i>
                            <h3 class="text-lg font-medium text-gray-900 mb-1">Belum ada data gaji</h3>
                            <p class="text-sm text-gray-500">Data gaji pada periode ini belum tersedia.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
